...
void loop()
{
  ...
  uint8_t uid[7];
  uint8_t uuid[4];
  uint8_t lenId = nfc.readCardSerial(uid);
  ...
  Serial.println();
  // uint8_t uuid[7] = { 0x04, 0x82, 0x4C, 0x0A, 0x63, 0x18, 0x90};
  // uint8_t uuid[4] = { 0x6D, 0x9B, 0x8E, 0x13 };
  // uint8_t uuid[4] = {0x0A, 0x63, 0x18, 0x90};
  // uint8_t uuid[4] = { 0xCD, 0xB3, 0x8D, 0x13 };
  ...
  Serial.println();
  uint8_t mKey[6] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
  int16_t noAuth = nfc.mifareAuthenticate(0, mKey, (0x61), uuid);
  ...
  {
    uint8_t mData[16];
    bool isRead = nfc.mifareBlockRead(2, mData);
    if (isRead)
    {
